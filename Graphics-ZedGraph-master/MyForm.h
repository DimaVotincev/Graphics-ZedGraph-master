...
static void solveDirichleForPuasson(
	...
	while (flag) {
		...
		for (int i = 0; i < n+1; ++i) {
			for (int j = 0; j < m + 1; ++j) {
				eps_S = std::max(eps_S, abs(V[i][j]-V_prev[i][j]));
			}
		}
...
#pragma endregion
	...
	private: void DrawTask() {
		...
		solveDirichleForPuasson(k_func, q_func, f_func, a, b, (double)n, mu1, mu2, ksi, vi, v2i, out_grid, out_grid2);
